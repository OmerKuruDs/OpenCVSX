...
def _make_button(action: QAction, parent: QWidget) -> QToolButton:
...
class ImageToolsPanel(QWidget):
    def __init__(
        ...
        *,
        ...
        view_layout.addWidget(_make_button(split_action, view_group))
        view_layout.addWidget(_make_button(downscale_action, view_group))
        layout.addWidget(view_group)

        roi_group = QGroupBox("ROI", self)
        roi_layout = QVBoxLayout(roi_group)
        roi_layout.setSpacing(6)
        roi_layout.addWidget(_make_button(select_roi_action, roi_group))
        roi_layout.addWidget(_make_button(clear_roi_action, roi_group))
        layout.addWidget(roi_group)

        paste_group = QGroupBox("Paste destination", self)
        paste_layout = QVBoxLayout(paste_group)
        paste_layout.setSpacing(6)
        paste_layout.addWidget(_make_button(randomize_paste_action, paste_group))
        paste_layout.addWidget(_make_button(clear_paste_action, paste_group))
